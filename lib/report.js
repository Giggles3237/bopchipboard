...
function changeValueSummary(unit) {
  if (!unit) return 'Not present';
  const lease = unit.leasePayment === null || unit.leasePayment === undefined
    ? 'N/A'
    : '$' + unit.leasePayment + '/mo';
  return [
    `Miles: ${milesFmt(unit.odometer)}`,
    `Sale: ${moneyFmt(unit.salePrice)}`,
    `Lease: ${lease}`,
    `Status: ${String(STATUS_LABELS[unit.status] || unit.status || 'N/A')}`,
  ].join('<br>');
}

function renderInventoryChangeSection(changes = [], { previousGeneratedAt = null } = {}) {
  const previousText = previousGeneratedAt
    ? `Compared with the report generated ${new Date(previousGeneratedAt).toLocaleString('en-US')}.`
    : 'No prior report was available, so every current unit is listed as new.';

  if (!changes.length) {
    return `
    <div data-loaner-supplemental="true" data-inventory-manager-report="true">
    <h2 style="font-family:${FONT};font-size:15px;color:${DARK};margin:28px 0 8px;">
      Inventory Manager change report
    </h2>
    <p style="font-family:${FONT};font-size:12px;color:${MUTED};margin:0 0 10px;">
      ${esc(previousText)} No stock-level miles, price, payment, or status changes were found.
    </p>
    </div>`;
  }

  const rows = changes.map((change, i) => `<tr data-brand="${esc(change.brand || brandOf(change.stockNumber, change.model))}" style="background:${i % 2 ? ROW_ALT : '#ffffff'};">
        <td ${td('font-weight:bold;white-space:nowrap;')}>${esc(change.stockNumber)}</td>
        <td ${td()}>${esc(change.model)}</td>
        <td ${td()}>${esc((change.types || []).join(', '))}</td>
        <td ${td('font-size:12px;line-height:1.4;')}>${changeValueSummary(change.previous)}</td>
        <td ${td('font-size:12px;line-height:1.4;font-weight:bold;')}>${changeValueSummary(change.current)}</td>
        <td ${td(`color:${AMBER_TX};font-weight:bold;`)}>${esc(change.action)}</td>
      </tr>`).join('');

  return `
    <div data-loaner-supplemental="true" data-inventory-manager-report="true">
    <h2 style="font-family:${FONT};font-size:15px;color:${DARK};margin:28px 0 8px;">
      Inventory Manager change report (${changes.length})
    </h2>
    <p style="font-family:${FONT};font-size:12px;color:${MUTED};margin:0 0 10px;">
      ${esc(previousText)} Use this table to update miles or review prices that moved since the last generated report.
    </p>
    <table role="presentation" cellpadding="0" cellspacing="0" class="sortable" style="border-collapse:collapse;font-family:${FONT};font-size:13px;color:${DARK};">
      <thead><tr>
        ...
        <th ${TH}>Changed<span class="arw"></span></th><th ${TH}>Previous</th><th ${TH}>Current</th><th ${TH}>Action<span class="arw"></span></th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>
    </div>`;
...
function renderSheet(report, settings, { reportDate = new Date(), includeDisclosures = false } = {}) {
  ...
  if (report.inventoryChanges) {
    sections.push(renderInventoryChangeSection(report.inventoryChanges, {
  ...
  const stats = [
    ...
    report.inventoryChanges?.length ? `<b>${report.inventoryChanges.length}</b> changed since last report` : '',
...
function renderInventoryManagerReport(changes = [], settings, {
  reportDate = new Date(),
  previousGeneratedAt = null,
} = {}) {
  const dateLong = reportDate.toLocaleDateString('en-US',
    { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
  const section = renderInventoryChangeSection(changes, { previousGeneratedAt });
  return `<div style="max-width:860px;margin:0 auto;background:#ffffff;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
    <tr>
      <td style="background:${DARK};padding:22px 24px;border-top:4px solid ${BMW_BLUE};">
    ...
    <tr><td style="padding:18px 24px 6px;">${section}</td></tr>
  </table>
</div>
${SORT_SCRIPT}
`;
