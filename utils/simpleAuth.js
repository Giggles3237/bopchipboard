const { DeviceCodeCredential } = require("@azure/identity");
const axios = require("axios");

// Configuration
const config = {
  tenantId: "fb123195-57d4-485c-8875-f5c0f8136c67",
  clientId: "58a504c8-0888-4eb1-928c-108c9d0c8083"
};

// SharePoint file information
const SHAREPOINT_CONFIG = {
  domain: "pandwbmw-my.sharepoint.com",
  userEmail: "chris.lasko@pandwforeigncars.com",
  fileId: "Ec0ILScNLXBPpvySJPg08uwBIiT90aMZ86eqbq47V5HyLg",
  fileName: "Get Ready.xlsx"
};

async function getDeviceCodeToken() {
  try {
    // This will show a code to enter at microsoft.com/devicelogin
    const credential = new DeviceCodeCredential({
      tenantId: config.tenantId,
      clientId: config.clientId
    });

    const tokenResponse = await credential.getToken([
      "https://graph.microsoft.com/Files.ReadWrite",
      "https://graph.microsoft.com/Sites.ReadWrite.All"
    ]);

    return tokenResponse.token;
  } catch (error) {
    console.error("Error getting device code token:", error.message);
    throw error;
  }
}

async function addRowToExcelDeviceCode(values) {
  try {
    const accessToken = await getDeviceCodeToken();
    
    // Use /me endpoint with device code authentication
    const url = `https://graph.microsoft.com/v1.0/me/drive/items/${SHAREPOINT_CONFIG.fileId}:/workbook/tables/Table1/rows/add`;

    const body = {
      values: [values]
    };

    const res = await axios.post(url, body, {
      headers: { Authorization: `Bearer ${accessToken}` }
    });

    console.log("Row added via device code auth:", res.data);
    return res.data;
  } catch (error) {
    console.error("Error adding row to Excel via device code auth:", error.response?.data || error.message);
    throw error;
  }
}

// Test function
async function testDeviceCodeAuth() {
  console.log('🔐 Testing device code authentication...\n');
  console.log('This will show you a code to enter at microsoft.com/devicelogin');
  console.log('Please sign in with your work account (chris.lasko@pandwforeigncars.com)\n');
  
  try {
    const testData = [
      "",                                // ID
      new Date().toISOString(),          // Start Time
      new Date().toISOString(),          // Completion Time
      "test@pandwforeigncars.com",       // Email
      "Test User",                       // Name
      "TEST123",                         // Stock#
      "Detail",                          // Location
      "Test Customer",                   // Customer Name
      "X5",                              // Model
      "TEST123456789",                   // Chassis
      "White",                           // Color
      "10000",                           // Miles
      "Test instructions",               // Instructions
      "Test comment",                    // Comments
      new Date(Date.now() + 86400000).toISOString().split('T')[0], // Due Date
      "3PM",                             // Promise Time
      "test.advisor@pandwforeigncars.com", // Advisor email
      "No"                               // Sold Unit
    ];
    
    const result = await addRowToExcelDeviceCode(testData);
    console.log('✅ Success! Row added:', result);
    
  } catch (error) {
    console.error('❌ Test failed:', error.message);
  }
}

module.exports = { 
  addRowToExcelDeviceCode, 
  getDeviceCodeToken,
  testDeviceCodeAuth 
}; 